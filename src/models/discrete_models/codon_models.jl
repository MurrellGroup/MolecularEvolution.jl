...
struct GeneticCode
    ...
    function GeneticCode(genetic_code::Dict{String,Char})
        ...
        string2sense = Dict{String,Int}()
        for cod in 1:length(sense_codons)
        ...
        codon_to_nuc_map = hcat([[(-1,-1,-1) for i in 1:length(sense_codons)] for i in 1:length(sense_codons)]...)

        for i in 1:length(sense_codons)
            for j in 1:length(sense_codons)
        ...
        codon2AA_pos = [findfirst(genetic_code[cod] .== amino_acids) for cod in sense_codons]

        new(genetic_code,codons,sense_codons,stop_codons,amino_acid_lookup,sense2all,string2sense,syn_positions,nonsyn_positions,amino_acids,codon2AA_pos,codon_to_nuc_map,length(sense_codons))
    end
end
...
function count_F3x4(seqs::Array{String})
    F3x4 = zeros(3,4)
    for k in 1:3
        pos1inds = [3*(i-1)+k for i in 1:Int(length(seqs[1])/3)]
...
function F3x4_eq_freqs(F3x4; genetic_code = universal_code)
    eq = [
        F3x4[1,nuc2num[c[1]]]*F3x4[2,nuc2num[c[2]]]*F3x4[3,nuc2num[c[3]]]
        for c in genetic_code.sense_codons]
    return eq ./ sum(eq)
end
...
function MG94_F3x4(alpha,beta,nuc_matrix,F3x4; genetic_code = universal_code)
    ...
    for p in genetic_code.syn_positions
        codon_matrix[p[1][1],p[1][2]] = alpha * nuc_matrix[p[2][2],p[2][3]] * F3x4[p[2][1],p[2][3]]
    end
    for p in genetic_code.nonsyn_positions
        codon_matrix[p[1][1],p[1][2]] = beta * nuc_matrix[p[2][2],p[2][3]] * F3x4[p[2][1],p[2][3]]
    end
    for i in 1:length(genetic_code.sense_codons)
...
function count_F61(seqs::Array{String}; code = universal_code)
    ...
    dic = proportionmap(vcat([[seq[3*(i-1)+1:3*(i-1)+3] for i in 1:Int(length(seq)/3)] for seq in seqs]...))
    for i in 1:length(F61)
...
function HB98_AAfit(alpha, nuc_matrix,AA_fitness; genetic_code = universal_code)
    ...
    for p in genetic_code.nonsyn_positions
        #duplicated code, because this model doesn't really use the syn/non-syn distinction
        diff = (AA_fitness[genetic_code.codon2AA_pos[p[1][2]]] - AA_fitness[genetic_code.codon2AA_pos[p[1][1]]])#/(1+AA_fitness[genetic_code.codon2AA_pos[p[1][1]]]) #Need to think about this term in the context of the approximation
...
mutable struct CodonPartition <: DiscretePartition
    ...
    function CodonPartition(sites; code = universal_code)
        new(zeros(length(code.sense_codons),sites),length(code.sense_codons),sites, zeros(sites))
    end
end
