...
function add_node_to_phylo_tree(molev_node::FelNode, phylo_tree, phylo_node, node_counter; copy_dict = true, data_function = (x -> Tuple{String, Float64}[]))
    ...
    for c in molev_node.children
        ...
        add_node_to_phylo_tree(c, phylo_tree, new_child, node_counter, data_function = data_function)
    end
end
...
function get_phylo_tree(molev_root::FelNode; data_function = (x -> Tuple{String, Float64}[]))
    ...
    add_node_to_phylo_tree(molev_root, phylo_tree, phylo_root, 1, data_function = data_function)
    return phylo_tree
end
...
"""
function savefig_tweakSVG(fname; plot = nothing, hack_bounding_box = true, new_viewbox = nothing, linecap_round = true)
