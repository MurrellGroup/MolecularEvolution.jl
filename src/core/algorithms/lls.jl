...
"""
    log_likelihood(tree::GeneralFelNode; partition_list = nothing)

Computed the log likelihood of this tree. Requires felsenstein!() to have been run.
"""
function log_likelihood(tree::GeneralFelNode, model_func; partition_list = 1:length(tree.message))
...
log_likelihood(tree::GeneralFelNode, model::BranchModel; partition_list = 1:length(tree.message)) = log_likelihood(tree, x -> [model], partition_list = partition_list)
log_likelihood(tree::GeneralFelNode, models::Vector{<:BranchModel}; partition_list = 1:length(tree.message)) = log_likelihood(tree, x -> models, partition_list = partition_list)


"""
    log_likelihood!(tree::GeneralFelNode, models; partition_list = nothing)

First re-computes the upward felsenstein pass, and then computes the log likelihood of this tree.
"""
function log_likelihood!(tree::GeneralFelNode, models; partition_list = 1:length(tree.message))
