...
function nni_optim!(
    ...
    for i = 1:length(node.children)
        ...
        nni_optim!(
            ...
            partition_list;
            acc_rule = acc_rule,
            nni_config_sampler = nni_config_sampler,
        )
    end
    #Then combine node.child_messages into node.message...
    for part in partition_list
...
function do_nni(
    ...
    if length(node.children) == 0 || node.parent === nothing
        ...
        sampled_config_ind = nni_config_sampler(nni_LLs)
        changed = sampled_config_ind != 1
...
partition_list (eg. 1:3 or [1,3,5]) lets you choose which partitions to run over (but you probably want to optimize tree topology with all models).
acc_rule allows you to specify a function that takes the current and proposed log likelihoods, and if true is returned the move is accepted.
nni_config_sampler allows you to randomly select a NNI based on the vector of log likelihoods of the possible interchanges, note that the current log likelihood is at index 1. 
"""
function nni_optim!(
    tree::FelNode,
    models;
...
nni_optim!(
    ...
    acc_rule = (x, y) -> x > y,
    nni_config_sampler = (x) -> argmax(x),
) = nni_optim!(tree, x -> models, partition_list=partition_list, acc_rule=acc_rule, nni_config_sampler = nni_config_sampler)
nni_optim!(
    tree::FelNode,
    model::BranchModel;
    partition_list = nothing,
    acc_rule = (x, y) -> x > y,
    nni_config_sampler = (x) -> argmax(x),
) = nni_optim!(tree, x -> [model], partition_list=partition_list, acc_rule=acc_rule, nni_config_sampler = nni_config_sampler)


function nni_optim_temp!(
    temp_message::Vector{<:Partition},
    message_to_set::Vector{<:Partition},
    node::FelNode,
    models,
    ...
    #-------------------
    if !isroot(node)
        nnid, exceed_sib, exceed_child = do_nni_temp(
            node,
            temp_message,
            models;
            partition_list = partition_list,
...
"""
    nni_optim_temp!(tree::FelNode, models; partition_list = nothing, tol = 1e-5)
...
nni_optim_temp!(
    ...
    acc_rule = (x, y) -> x > y,
    sampler = (x) ->  (true, argmax(x[2:end]) + 1),
) = nni_optim_temp!(tree, x -> models, partition_list=partition_list, acc_rule=acc_rule, sampler = sampler)
nni_optim_temp!(
    tree::FelNode,
    model::BranchModel;
    partition_list = nothing,
    acc_rule = (x, y) -> x > y,
    sampler = (x) ->  (true, argmax(x[2:end]) + 1),
) = nni_optim_temp!(tree, x -> [model], partition_list=partition_list, acc_rule=acc_rule, sampler = sampler)
