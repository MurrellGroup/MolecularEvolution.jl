...
function _mapreduce(start_node::AbstractTreeNode, map_reduce::T, aggregator, queue_ordering_func!) where {T<:Function}
    queue = [(start_node, start_node)]
    
    while length(queue) > 0
        curr_node, prev_node = queue_ordering_func!(queue)
        parent_and_children = filter(x->!isnothing(x), vcat(curr_node.parent, curr_node.children))
        new_queue_elems = [(next_node, curr_node) for next_node in filter(x->x != prev_node, parent_and_children)]
        #println(queue)
...
"""
function bfs_mapreduce(start_node::AbstractTreeNode, map_reduce::T, aggregator) where {T<:Function}
...
"""
function dfs_mapreduce(start_node::AbstractTreeNode, map_reduce::T, aggregator) where {T<:Function}
...
function node_distances(from_node::AbstractTreeNode)::Dict{<:AbstractTreeNode, Float64}
    function map_reduce(curr_node::AbstractTreeNode, prev_node::AbstractTreeNode, aggregator::Dict{<:AbstractTreeNode, Float64})
...
function shortest_path_between_nodes(node1::FelNode, node2::FelNode)::Tuple{Array{<:FelNode,1},Array{<:FelNode,1}}
    ...
    i = 0
    while i < length(parent_list1) && i < length(parent_list2) && parent_list1[end-i] == parent_list2[end-i]
        i += 1
    end
...
function midpoint(root::FelNode)::Tuple{<:FelNode, Float64}
    ...
    remaining_length = (total_length/2) - (side_length1 > side_length2 ? side_length2 : side_length1)
